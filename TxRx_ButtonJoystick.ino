...
//Stepper Motors
bool initPos = 1;
int clawAngle = 20; // set to initial position of the claw
int armAngle = 70; //set to initial position of arm
int clawChange = clawAngle;
int armChange = armAngle;
Servo Claw; //initializing servo objects
Servo Arm;
...
void setup() {
 Serial.begin(115200);
  ...
  if (WifiSerial.getPortType() == WifiPortType::Transmitter || WifiSerial.getPortType() == WifiPortType::Emulator){
    b1Pin = 3; //vars set to pins on CoLe Bot
    b2Pin = 4;
    b3Pin = 5;
    b4Pin = 6;
    JPin = 2;
  ...
  if ((WifiSerial.getPortType() == WifiPortType::Receiver || WifiSerial.getPortType() == WifiPortType::Emulator)){
    ...
    //attching servomotors and settinng them to their intial positions
    Claw.attach(A1);
    Arm.attach(A0);
...
void loop() {
  ...
  if ((WifiSerial.getPortType() == WifiPortType::Receiver || WifiSerial.getPortType() == WifiPortType::Emulator) && WifiSerial.checkForData()) {
    ...
    const int rSpeed = map(data.joystickY, 540,1023,0,255); //lspeed and rspeed is actually front and back,they're switched
    ...
    if(data.button3 == LOW){ // open claw //5
      clawAngle = clawAngle -5;
    }

    if(data.button2 == LOW){ // close claw //4
      clawAngle = clawAngle + 5;
    }

    if(data.button4 == LOW){ // lower arm//3 
      armAngle = armAngle - 5;
    }

    if(data.button1 == LOW){ // raise arm //6
      armAngle = armAngle + 5;
    }

    if(data.joystickButton == LOW){ //use the joystick button to trigger dance since not being used
      dance();
    }
...
void dataCheck(){ //function to print all variables, used to see that all data is being input correctlly on tx and recieved correctly on rx
...
void dance(){ //little dance for the robot

  digitalWrite(Rdrive1A, LOW); //spin left
  digitalWrite(Rdrive2A, HIGH);
  digitalWrite(Ldrive1A, LOW);
  digitalWrite(Ldrive2A, HIGH);

  analogWrite(Rena1, 200); //speed just below max for safety
  analogWrite(Lena1, 200);

  for(int j = 0; j < 2; j++){   //arm goes up then down, done twice while spinninh
    for(int i = 52; i <= 125; i++){
      Arm.write(i);
      delay(10);
    }
    for(int i = 125; i >= 52; i--){
      Arm.write(i);
      delay(10);
    }
  }
  ...
  digitalWrite(Rdrive1A, HIGH);  //spin to the right and repeat
  digitalWrite(Rdrive2A, LOW);
  digitalWrite(Ldrive1A, HIGH);
  digitalWrite(Ldrive2A, LOW);

  analogWrite(Rena1, 200);
  analogWrite(Lena1, 200);

  for(int j = 0; j < 2; j++){ //repeats arm dance
    for(int i = 52; i <= 125; i++){
      Arm.write(i);
      delay(10);  
    for(int i = 125; i >= 52; i--){
  ...
  digitalWrite(Rdrive1A, LOW); //backwaards
  digitalWrite(Rdrive2A, HIGH);
  digitalWrite(Ldrive1A, HIGH);
  digitalWrite(Ldrive2A, LOW);
  ...
  analogWrite(Rena1, 0); //stop here at final movement
  analogWrite(Lena1, 0);

  Arm.write(90);
  Claw.write(40); //putting YEAH by usher to w dance
}
