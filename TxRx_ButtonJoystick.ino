...
void loop() {
  ...
  if ((WifiSerial.getPortType() == WifiPortType::Receiver || WifiSerial.getPortType() == WifiPortType::Emulator) && WifiSerial.checkForData()) {
    ...
    const int fwdSpeed = map(data.joystickX, 540,1023,0,125); //Find proportional value of 1028 as percentage of 255 for analogWrite

    const int bckSpeed = map(data.joystickX, 480,0,0,125);
...
void dance(){ //make sure to fix with delays, wasn't working before
  ...
  analogWrite(Rena1, 200); //just going to use numbers for speed

  analogWrite(Lena1, 200);

 

  for(int j = 0; j < 2; j++){   //arm movement, nested to make it simpler
